...
// ---------------------------------------------------------------------------
// 3.5. RunPod (SDXL Turbo) — se intenta PRIMERO, antes que Hugging Face.
//      Ajustar la lectura de `data.output` según el formato real que
//      devuelva tu worker (confirmable probando el endpoint desde el
//      dashboard de RunPod).
// ---------------------------------------------------------------------------
async function tryRunPod(prompt) {
  ...
  return { buffer, contentType: 'image/png', modelUsed: 'runpod-sdxl-turbo' };
}
...
export default async function handler(req, res) {
  ...
  try {
    buffer = (await tryRunPod(prompt)).buffer;
    contentType = 'image/png';
    modelUsed = 'runpod-sdxl-turbo';
  } catch (runpodErr) {
